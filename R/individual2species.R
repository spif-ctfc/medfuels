...
individual2species<-function(x, sampledarea = 20, maxcover = 100) {
  x = as.data.frame(x)
  vars = names(x)
  ...
  for(i in 1:length(plots)) {
    ...
    shpc = data.frame(plot = rep(plots[i], length(species)),
                      species = species,
                      H = tapply(pc$H, species, FUN=mean, na.rm=T),
                      C = 100*tapply(area, species, FUN=sum)/sampledarea)
    row.names(shpc) = NULL
